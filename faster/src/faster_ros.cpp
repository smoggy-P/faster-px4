...
FasterRos::FasterRos(ros::NodeHandle nh) : nh_(nh)
{
  ...
  */
  ...
  pub_goal_ = nh_.advertise<trajectory_msgs::JointTrajectoryPoint>("/pva_setpoint", 1);
...
void FasterRos::stateCB(const nav_msgs::Odometry::ConstPtr& msg)
{
  state state_tmp;
...
void FasterRos::pubCB(const ros::TimerEvent& e)
{
  ...
  {
    trajectory_msgs::JointTrajectoryPoint quadGoal;
    ...
    quadGoal.positions.push_back(eigen2rosvector(next_goal.pos).x);
    quadGoal.positions.push_back(eigen2rosvector(next_goal.pos).y);
    quadGoal.positions.push_back(eigen2rosvector(next_goal.pos).z);
    quadGoal.positions.push_back(next_goal.yaw);
 
    quadGoal.velocities.push_back(eigen2rosvector(next_goal.vel).x);
    quadGoal.velocities.push_back(eigen2rosvector(next_goal.vel).y);
    quadGoal.velocities.push_back(eigen2rosvector(next_goal.vel).z);

    quadGoal.accelerations.push_back(eigen2rosvector(next_goal.accel).x);
    quadGoal.accelerations.push_back(eigen2rosvector(next_goal.accel).y);
    quadGoal.accelerations.push_back(eigen2rosvector(next_goal.accel).z);

    pub_goal_.publish(quadGoal);

    setpoint_.header.stamp = ros::Time::now();
    setpoint_.pose.position.x = quadGoal.positions[0];
    setpoint_.pose.position.y = quadGoal.positions[1];
